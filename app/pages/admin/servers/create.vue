...
const { data: nestsData } = await useAsyncData('admin-nests-for-server', () =>
  requestFetch<{ data: Nest[] }>('/api/admin/nests'),
);

const { data: nodesData } = await useAsyncData('admin-nodes-for-server', () =>
...
watch(
  ...
  async (eggId) => {
    ...
    try {
      const response = await $fetch<{ data: Egg & { variables: EggVariable[] } }>(
        `/api/admin/eggs/${eggId}`,
      );
...
watch(
  ...
  async (nodeId) => {
    ...
    try {
      ...
      }>(`/api/admin/wings/nodes/${nodeId}/allocations`, {
        query: { perPage: 1000 },
      });
...
async function createServer() {
  ...
  try {
    ...
    const validationErrors = (err as any)?.data?.errors as
        ...
        <UCard>
          ...
          <div v-if="currentStep === 1" class="space-y-4">
            <UFormField :label="t('admin.servers.create.serverName')" name="name" required>
              <UInput
                v-model="form.name"
                :placeholder="t('admin.servers.create.serverNamePlaceholder')"
                required
                class="w-full"
              />
              <template #help>
            ...
            <UFormField :label="t('admin.servers.create.description')" name="description">
              <UTextarea
                ...
                :placeholder="t('admin.servers.create.descriptionPlaceholder')"
                class="w-full"
              />
            </UFormField>
          ...
          <div v-if="currentStep === 2" class="space-y-4">
            ...
            <UFormField
              v-if="form.nestId"
              :label="t('admin.servers.create.selectEgg')"
              name="eggId"
              required
            >
              <USelect
            ...
            <UAlert v-if="selectedEgg" color="primary" icon="i-lucide-info">
              ...
              <template #description>
                ...
                <p class="mt-2 text-xs">
                  {{ t('admin.servers.create.dockerImage') }}:
                  <code class="rounded bg-primary/10 px-1">{{ selectedEgg.dockerImage }}</code>
                </p>
              </template>
            </UAlert>
          </div>
          ...
          <div v-if="currentStep === 3" class="space-y-4">
            ...
            <div
              v-if="nodes.length === 0"
              class="rounded-lg border border-warning bg-warning/10 p-4"
            >
              <div class="flex items-start gap-3">
                <UIcon name="i-lucide-alert-triangle" class="size-5 text-warning" />
                <div>
                  <p class="font-medium text-warning">
                    {{ t('admin.servers.create.noNodesAvailable') }}
                  </p>
                  <p class="mt-1 text-sm text-muted-foreground">
            ...
            <div class="grid grid-cols-2 gap-4">
              <UFormField :label="t('admin.servers.create.memory')" name="memory" required>
                <UInput
                  ...
                  type="number"
                  :placeholder="t('admin.servers.create.memoryPlaceholder')"
                  required
                  class="w-full"
                />
                <template #help>
              ...
              <UFormField :label="t('admin.servers.create.swap')" name="swap">
                <UInput
                  ...
                  type="number"
                  :placeholder="t('admin.servers.create.swapPlaceholder')"
                  class="w-full"
                />
                <template #help>
              ...
              <UFormField :label="t('admin.servers.create.diskSpace')" name="disk" required>
                <UInput
                  ...
                  type="number"
                  :placeholder="t('admin.servers.create.diskSpacePlaceholder')"
                  required
                  class="w-full"
                />
                <template #help>
              ...
              <UFormField :label="t('admin.servers.create.cpuLimit')" name="cpu" required>
                <UInput
                  ...
                  type="number"
                  :placeholder="t('admin.servers.create.cpuLimitPlaceholder')"
                  required
                  class="w-full"
                />
                <template #help>
              ...
              <UFormField :label="t('admin.servers.create.ioWeight')" name="io">
                <UInput
                  ...
                  type="number"
                  :placeholder="t('admin.servers.create.ioWeightPlaceholder')"
                  class="w-full"
                />
                <template #help>
              ...
              <UFormField :label="t('admin.servers.create.cpuThreads')" name="threads">
                <UInput
                  ...
                  :placeholder="t('admin.servers.create.cpuThreadsPlaceholder')"
                  class="w-full"
                />
                <template #help>
              ...
              <div class="grid grid-cols-3 gap-4">
                <UFormField :label="t('admin.servers.create.databaseLimit')" name="databases">
                  <UInputNumber
                    ...
                    :min="0"
                    :placeholder="t('admin.servers.create.databaseLimitPlaceholder')"
                    class="w-full"
                  />
                  <template #help>
                ...
                <UFormField :label="t('admin.servers.create.allocationLimit')" name="allocations">
                  <UInputNumber
                    ...
                    :min="0"
                    :placeholder="t('admin.servers.create.allocationLimitPlaceholder')"
                    class="w-full"
                  />
                  <template #help>
                ...
                <UFormField :label="t('admin.servers.create.backupLimit')" name="backups">
                  <UInputNumber
                    ...
                    :min="0"
                    :placeholder="t('admin.servers.create.backupLimitPlaceholder')"
                    class="w-full"
                  />
                  <template #help>
          ...
          <div v-if="currentStep === 5" class="space-y-4">
            <UFormField
              :label="t('admin.servers.create.primaryAllocation')"
              name="allocationId"
              required
            >
              <USelectMenu
                ...
                :disabled="
                  !form.nodeId || isLoadingAllocations || availableAllocations.length === 0
                "
              ...
              <template #help>
                <span v-if="!form.nodeId">{{
                  t('admin.servers.create.primaryAllocationHelpSelectNode')
                }}</span>
                <span v-else-if="isLoadingAllocations">{{
                  t('admin.servers.create.primaryAllocationHelpLoading')
                }}</span>
                <span v-else-if="availableAllocations.length === 0">{{
                  t('admin.servers.create.primaryAllocationHelpNoAllocations')
                }}</span>
            ...
            <UAlert
              v-if="form.nodeId && availableAllocations.length === 0 && !isLoadingAllocations"
              color="warning"
            ...
            <UFormField :label="t('admin.servers.create.startupCommand')" name="startup">
              <UTextarea
                ...
                :placeholder="
                  selectedEgg?.startup || t('admin.servers.create.startupCommandPlaceholder')
                "
                class="w-full"
              />
              <template #help>
            ...
            <div v-if="eggVariables.length > 0" class="space-y-3">
              <h3 class="text-sm font-semibold">
                {{ t('admin.servers.create.environmentVariables') }}
              </h3>
            ...
            <div class="flex gap-4">
              ...
              <UFormField
                :label="t('admin.servers.create.startAfterInstall')"
