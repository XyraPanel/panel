...
const { data: eggsData } = await useFetch<{
  data: { id: string; name: string; nestName?: string }[];
}>('/api/admin/eggs', {
              ...
              >
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <UBadge v-if="mount.userMountable" size="xs" color="primary"
      ...
      <template #body>
        <form class="space-y-4" @submit.prevent="handleSubmit">
          <UFormField :label="t('admin.mounts.name')" name="name" required>
            <UInput
              v-model="form.name"
              :placeholder="t('admin.mounts.namePlaceholder')"
              required
          ...
          <UFormField :label="t('common.description')" name="description">
            <UTextarea
              ...
              :placeholder="t('admin.mounts.descriptionPlaceholder')"
              :disabled="isSubmitting"
              class="w-full"
            />
          </UFormField>
          ...
          <UFormField :label="t('admin.mounts.source')" name="source" required>
            <UInput
              v-model="form.source"
              :placeholder="t('admin.mounts.sourcePlaceholder')"
              required
              :disabled="isSubmitting"
              class="w-full"
            />
          ...
          <UFormField :label="t('admin.mounts.target')" name="target" required>
            <UInput
              v-model="form.target"
              :placeholder="t('admin.mounts.targetPlaceholder')"
              required
              :disabled="isSubmitting"
              class="w-full"
            />
    ...
    >
      <template #body>
        ...
        <div v-if="mountToDelete" class="rounded-md bg-muted p-3 text-sm">
          <p class="font-medium">
            {{ t('common.name') }}: <span class="text-foreground">{{ mountToDelete.name }}</span>
          </p>
          <p class="text-muted-foreground mt-2">
            {{ mountToDelete.source }} → {{ mountToDelete.target }}
          </p>
        </div>
      </template>
