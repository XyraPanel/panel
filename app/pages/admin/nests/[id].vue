...
const {
  ...
  error,
  refresh,
} = await useAsyncData(`admin-nest-${nestId.value}`, () =>
...
async function handleCreateEgg() {
  if (
    !eggForm.value.name ||
    !eggForm.value.author ||
    !eggForm.value.dockerImage ||
    !eggForm.value.startup
  ) {
...
<template>
  <UPage>
        ...
        <section class="space-y-6">
          ...
          <template v-else-if="nest">
            ...
            <UCard>
              <template #header>
                <div class="flex items-center justify-between">
                  ...
                  <UBadge color="neutral" variant="outline" size="sm"
                    >{{ eggs.length }} {{ t('common.all') }}</UBadge
                  >
                </div>
              </template>
              ...
              >
                <UButton class="mt-4" size="sm" @click="openCreateEggModal">{{
                  t('admin.nests.createEgg.createFirstEgg')
                }}</UButton>
              </UEmpty>
    ...
    <UModal
      v-model:open="showCreateEggModal"
      :title="t('admin.nests.createEgg.createEgg')"
        ...
        <form class="space-y-4" @submit.prevent="handleCreateEgg">
          <UFormField :label="t('common.name')" name="name" required>
            <UInput
              ...
              :placeholder="t('admin.nests.createEgg.namePlaceholder')"
              required
          ...
          <UFormField :label="t('common.description')" name="description">
            <UTextarea
              ...
              :placeholder="t('admin.nests.createEgg.descriptionPlaceholder')"
              :disabled="isSubmitting"
              class="w-full"
            />
          </UFormField>
          ...
          <UFormField :label="t('admin.nests.createEgg.dockerImage')" name="dockerImage" required>
            <UInput
              v-model="eggForm.dockerImage"
              :placeholder="t('admin.nests.createEgg.dockerImagePlaceholder')"
              required
              :disabled="isSubmitting"
              class="w-full"
            />
          ...
          <UFormField :label="t('admin.nests.createEgg.startupCommand')" name="startup" required>
            <UTextarea
              v-model="eggForm.startup"
              :placeholder="t('admin.nests.createEgg.startupCommandPlaceholder')"
              required
              :disabled="isSubmitting"
              class="w-full"
            />
    ...
    <UModal
      v-model:open="showImportEggModal"
      :title="t('admin.nests.createEgg.importEgg')"
