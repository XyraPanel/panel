...
<template>
  <UPage>
                ...
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <UBadge v-if="host.databaseCount > 0" size="xs" color="neutral">
                      {{ host.databaseCount }} / {{ host.maxDatabases || '∞' }}
                      {{ t('admin.databaseHosts.databases') }}
                    </UBadge>
                  </div>
    ...
    <UModal
      ...
      :title="t('admin.databaseHosts.addDatabaseHost')"
      ...
      <template #body>
        <form class="space-y-4" @submit.prevent="handleSubmit">
          <UFormField :label="t('common.name')" name="name" required>
            <UInput
              v-model="form.name"
              :placeholder="t('admin.databaseHosts.namePlaceholder')"
              required
          ...
          <div class="grid grid-cols-2 gap-4">
            <UFormField :label="t('admin.databaseHosts.hostname')" name="hostname" required>
              <UInput
                ...
                :placeholder="t('admin.databaseHosts.hostnamePlaceholder')"
                required
                :disabled="isSubmitting"
                class="w-full"
              />
            ...
            <UFormField :label="t('admin.databaseHosts.port')" name="port" required>
              <UInput
                ...
                type="number"
                :placeholder="t('admin.databaseHosts.portPlaceholder')"
                required
                :disabled="isSubmitting"
                class="w-full"
              />
          ...
          <div class="grid grid-cols-2 gap-4">
            <UFormField :label="t('auth.username')" name="username" required>
              <UInput
                ...
                :placeholder="t('admin.databaseHosts.usernamePlaceholder')"
                required
                :disabled="isSubmitting"
                class="w-full"
              />
            ...
            <UFormField :label="t('auth.password')" name="password" required>
              <UInput
                ...
                type="password"
                :placeholder="t('admin.databaseHosts.passwordPlaceholder')"
                required
                :disabled="isSubmitting"
                class="w-full"
              />
          ...
          <UFormField :label="t('admin.databaseHosts.defaultDatabase')" name="database">
            <UInput
              ...
              :placeholder="t('admin.databaseHosts.defaultDatabasePlaceholder')"
              :disabled="isSubmitting"
              class="w-full"
            />
            <template #help>
          ...
          <UFormField :label="t('admin.databaseHosts.maxDatabases')" name="maxDatabases">
            <UInput
              v-model.number="form.maxDatabases"
              type="number"
              :placeholder="t('admin.databaseHosts.maxDatabasesPlaceholder')"
              :disabled="isSubmitting"
              class="w-full"
            />
            <template #help>
    ...
    >
      <template #body>
        ...
        <div v-if="hostToDelete" class="rounded-md bg-muted p-3 text-sm">
          <p class="font-medium">
            {{ t('common.name') }}: <span class="text-foreground">{{ hostToDelete.name }}</span>
          </p>
