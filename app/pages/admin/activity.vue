      ...
      <UContainer>
        <section class="space-y-6">
          <UCard :ui="{ body: 'space-y-3' }">
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <UButton icon="i-lucide-download" color="neutral" variant="outline"
                    :disabled="pending || activities.length === 0" @click="exportCsv">
            ...
            <UEmpty v-else-if="activities.length === 0" icon="i-lucide-activity"
              :title="t('admin.activity.noActivityYet')" :description="t('admin.activity.noActivityYetDescription')" />
            <template v-else>
              <div class="space-y-3">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="mt-1 flex flex-wrap items-center gap-2">
                        <span class="text-xs text-muted-foreground">
                          ...
                          <NuxtLink v-if="entry.actorUserId" :to="`/admin/users/${entry.actorUserId}`"
                            class="font-medium text-primary hover:underline" @click.stop>
                            {{ entry.actorDisplay || entry.actor }}
                          </NuxtLink>
                          <span v-else class="font-medium">{{ entry.actorDisplay || entry.actor }}</span>
                        </span>
                        <span v-if="entry.target && !entry.target.startsWith('user#')"
                    ...
                    <div class="space-y-2">
                      <div class="flex items-center justify-between mb-2">
                        <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{
                          t('admin.activity.auditLogEntry') }}</p>
                      ...
                      <pre
                        class="text-xs font-mono bg-default rounded-lg p-3 overflow-x-auto border border-default"><code>{{ formatJson(getFullAuditData(entry)) }}</code></pre>
                    </div>
                  </div>
                </div>
              </div>

              <div v-if="pagination && pagination.total > itemsPerPage"
                ...
                <UPagination v-model:page="currentPage" :total="pagination.total" :items-per-page="itemsPerPage"
                  size="sm" />
              </div>
            </template>
          </UCard>
